...
const defaultConfig = {
  ...
  folderGroups: {
    ...
    tercaQuinta: "assets/audio/musicas/Terca-Quinta",
    sexta: "assets/audio/musicas/Sexta"
  }
};
...
async function renderMusicByDay() {
  ...
  const qtdSegundaQuarta = (await getTracksForDay("segunda")).length;
  const qtdTercaQuinta = (await getTracksForDay("terca")).length;
  const qtdSexta = (await getTracksForDay("sexta")).length;

  if (countSegundaQuarta) {
    countSegundaQuarta.textContent = `${qtdSegundaQuarta} musicas`;
  }
  if (countTercaQuinta) {
    countTercaQuinta.textContent = `${qtdTercaQuinta} musicas`;
  }
  if (countSexta) {
    countSexta.textContent = `${qtdSexta} musicas`;
  }
  ...
  if (todayKey) {
    ...
    todayItem.innerHTML = `
      ...
      <a href="#">${todayCount} musicas configuradas</a>
    `;
  ...
  for (const day of ["segunda", "terca", "quarta", "quinta", "sexta"]) {
    ...
    item.innerHTML = `
      <p>${DAY_LABELS[day]}</p>
      <a href="#">${tracks.length} musicas em ${getFolderForDay(day)}</a>
    `;
